...
class Tree {
    ...
    public function get_child($myid) {
        ...
        $this->level++;
        return !empty($newarr) ? $newarr : false;
    }
    ...
    /**
     * 表示获得指定 ID下的的所有下级分类id
     * @param int ID，表示获得这个ID下的所有子级
     * @param int 被选中的ID，比如在做树型下拉框的时候需要用到
     * @return string
     * ,1,2,3,4 需要去掉 第一个逗号来使用
     */
    public function get_tree_array_ids($myid) {
    	$number = 1;
        //一级栏目
        ...
        if (is_array($child) ) {
            ...
            foreach ($child as $id => $value) {
                @extract($value);
                $this->ret .= ','.$value['id'];
                $this->ret .= $this->get_tree_array_ids($id);
                $number++;
            }
        }
        return $this->ret;
    ...
    public function get_tree_select($myid, $str, $sid = 0, $adds = '', $str_group = '') {
    	$number = 1;
    	//一级栏目
    	$child = $this->get_child($myid);
    	if (is_array($child)) {
    		...
    		foreach ($child as $id => $value) {
    			...
    			$this->ret .= $nstr;
    			$nbsp = $this->nbsp;
    			$this->get_tree_select($id, $str, $sid, $adds . $k . $nbsp, $str_group);
    			$number++;
    		}
    	}
    	return $this->ret;
