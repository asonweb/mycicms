...
class Ion_auth
{
	 ...
	 * 
	 *
	 * @param string $option 默认选项
	 * @return array 用于生成下拉菜单
	 */
	public function groups_array($option=null){
		$result = $this->ion_auth_model->groups()->result_array();
		if (isset($option)) {
			$arr[''] = $option;
		}
		foreach ($result as $row){
			$arr[$row['id']] = $row['role_description'];
		}
		return $arr;
	}
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		{
			$users_groups = $this->ion_auth_model->get_users_groups($id)->result_array();
			
			$groups_array = array();
			foreach ($users_groups as $group)
			{
				$groups_array[$group['id']] = $group['role_code'];
			}
