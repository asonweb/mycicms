...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$hash_password_db = $query->row_array();
	 ...
	 * 
	 * 1.验证账号：是否已存在或不合法
	 * 2.验证角色：是否设定默认角色，是否有传参。默认角色是否存在数据表中
	 * 3.默认角色数据：从数据库查询得到
	 * 4.
	 *
	 * @return bool
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		// 检查数据库是否存在默认的角色名称
		$query = $this->db->get_where($this->tables['groups'],array('role_code' => $this->config->item('default_group', 'ion_auth')),1)->row_array();
		...
		// 用户数据
		$data = array(
		    $this->identity_column   => auto_charset($identity),
		    'password'   => $password,
		    'email'      => $email,
		...
		if (!empty($groups))
		{
			...
			{
				$this->add_to_group($group,(int) $id);
			}
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if (empty($identity) || empty($password))//身份空 密码空，false
		{
		...
		if ($query->num_rows() === 1)
		{
			$user = $query->row_array();//查询到的用户数据

			$password = $this->hash_password_db($user['id'], $password);//输入的密码 和查询到的用户实体是否匹配

			if ($password === TRUE)//匹配成功
			{
				if ($user['active'] == 0) //用户未激活
				{
					$this->trigger_events('post_login_unsuccessful');
				...
				$this->set_session($user);//登陆成功，设置会话状态

				$this->update_last_login((int)$user['id']);//更新最后登陆时间

				$this->clear_login_attempts($identity);

				if ($remember && $this->config->item('remember_users', 'ion_auth'))//记住用户？
				{
	...
	public function user($id = NULL)
	{
		...
		$this->where($this->tables['users'].'.id', intval($id));
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.role_code, '.$this->tables['groups'].'.role_name')
		->where($this->tables['users_groups'].'.'.$this->join['users'], intval($id) )
		...
		foreach ($group_ids as $group_id)
		{
			if ($this->db->insert($this->tables['users_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id)))
			{
				if (isset($this->_cache_groups[$group_id])) {
					...
					$group = $this->group((int)$group_id)->result_array();
					$group_name = $group[0]['role_code'];
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $data, array('id' => (int)$user['id']));
	...
	**/
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time()), array('id' => (int) $id));
	 ...
	 * 记住用户
	 * 
	 * 生成一个salt，remember_code 保存了一个 salt,输出 cookie保存在浏览器
	 * @param int $id
	 * @return bool
	 * @author Ben Edmunds
	 **/
	public function remember_user($id)
	{
		...
		$result = $this->db->update($this->tables['users'], array('remember_code' => $salt), array('id' => intval($id)));
	...
	/**
	 * 自动登陆记住的用户login_remembed_user
	 * 
	 * 通过cookie的字符串，匹配表的 remember_code，匹配成功则更新用户session状态，（自动登陆）
	 * 设置用户session
	 * @return bool
	 * @author Ben Edmunds
	 **/
	...
	*/
	 ...
	 * @author aditya menon
	 **/
	public function update_group($group_id = FALSE, $role_code = FALSE, $additional_data = array())
	{
		...
		if (!empty($role_code))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['groups'], array('role_code' => $role_code))->row_array();
        ...
        $group = $this->db->get_where($this->tables['groups'], array('id' => (int)$group_id))->row_array();
        if($this->config->item('admin_group', 'ion_auth') === $group['role_code'] && $group_name !== $group['role_code'])
        {
		...
		$this->db->update($this->tables['groups'], $data, array('id' => (int)$group_id));
	...
	**/
	...
	{
		$fields = $this->db->limit(1)->get($table)->list_fields();
